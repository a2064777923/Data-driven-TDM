<template>
  <motion-button
    v-bind="animationProps"
    :class="cn(
      'relative rounded-lg px-6 py-2 font-medium backdrop-blur-xl transition-shadow duration-300 ease-in-out hover:shadow dark:bg-[radial-gradient(circle_at_50%_0%,hsl(var(--primary)/10%)_0%,transparent_60%)] dark:hover:shadow-[0_0_20px_hsl(var(--primary)/10%)]',
      className
    )"
    ref="buttonRef"
    v-on="attrs"
  >
    <span
      class="relative block size-full text-sm uppercase tracking-wide text-[rgb(0,0,0,65%)] dark:font-light dark:text-[rgb(255,255,255,90%)]"
      :style="{
        maskImage: `linear-gradient(-75deg,hsl(var(--primary)) calc(var(--x) + 20%),transparent calc(var(--x) + 30%),hsl(var(--primary)) calc(var(--x) + 100%))`,
      }"
    >
      <slot></slot>
    </span>
    <span
      :style="{
        mask: 'linear-gradient(rgb(0,0,0), rgb(0,0,0)) content-box,linear-gradient(rgb(0,0,0), rgb(0,0,0))',
        maskComposite: 'exclude',
      }"
      class="absolute inset-0 z-10 block rounded-[inherit] bg-[linear-gradient(-75deg,hsl(var(--primary)/10%) calc(var(--x)+20%),hsl(var(--primary)/50%) calc(var(--x)+25%),hsl(var(--primary)/10%) calc(var(--x)+100%))] p-px"
    ></span>
  </motion-button>
</template>

<script lang="ts">
import { defineComponent, ref } from "vue";
import { motion } from "vue-motion"; // 使用 Vue 的 framer-motion 对应版本
import { cn } from "@/lib/utils"; // 或者你自己的 cn 函数

export default defineComponent({
  name: "ShinyButton",
  props: {
    className: {
      type: String,
      default: '',
    },
  },
  setup(props, { attrs }) {
    const buttonRef = ref<HTMLElement | null>(null);

    const animationProps = {
      initial: { "--x": "100%", scale: 0.8 },
      animate: { "--x": "-100%", scale: 1 },
      whileTap: { scale: 0.95 },
      transition: {
        repeat: Infinity,
        repeatType: "loop",
        repeatDelay: 1,
        type: "spring",
        stiffness: 20,
        damping: 15,
        mass: 2,
        scale: {
          type: "spring",
          stiffness: 200,
          damping: 5,
          mass: 0.5,
        },
      },
    };

    return {
      buttonRef,
      animationProps,
      cn,
      className: props.className,
      attrs,
    };
  },
});
</script>

<style scoped>
/* 可根据需要添加样式 */
</style>
