...
const MacauRegion = ["花地玛堂区","圣安多尼堂区","大堂区","望德堂区","风顺堂区","嘉模堂区","圣方济各堂区","路氹填海区"];
...
function getRandomMacauRegion() {
  const randomIndex = Mock.Random.integer(0, MacauRegion.length - 1);
  return MacauRegion[randomIndex];
}
...
export default [
    ...
    },
    {
        ...
        response: (options: any) => {
            ...
            //不是澳門的时候
            if (params.regionCode && !["Macau"].includes(params.regionCode)) {
                const a = Mock.mock({
                    success: true,
                    data: {
                        "dataList|100": [
                            {
                                name: getRandomMacauRegion(),
                                value: '@integer(1, 1000)'
                            }
                        ],
                        regionCode: params.regionCode,//-代表澳門
                    }
                })
                return a
            } else {
