...
/**这段代码是一个用于配置地图可视化的函数，optionHandle。
它接受三个参数：regionCode、list和mapData，并返回一个用于生成地图的配置对象。
**/
...
export const optionHandle = (regionCode: string,
    list: object[],
    mapData: MapdataType[]) => { //散點的數據
    let top = 45;
    let zoom = 1;
    return {
        backgroundColor: "rgba(0,0,0,0)", //地圖背景顏色
        tooltip: {
            show: false,
        },
        legend: {
        ...
        series: [
            {
                ...
                itemStyle: {
                    borderColor: "rgba(247, 235, 248, .8)",
                    borderWidth: 1,
                    areaColor: {
                        type: "radial",
                        x: 0.5,
                        ...
                        colorStops: [
                            {
                                offset: 0,
                                color: "rgba(247, 235, 248, 0)", // 0% 处的颜色
                            },
                            {
                                offset: 1,
                                color: "rgba(247, 235, 248, .2)", // 100% 处的颜色
                            },
                        ],
                        globalCoord: false, // 缺为 false
                    },
